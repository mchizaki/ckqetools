...
QEでの分散の計算で出力されるデータには，各波数 $q$ における固有値が書き込まれる。ところが，この固有値のデータは単に大きさ順に並んでいるだけである。適切な分散の図を得るためには，ある $q$ と隣の $q$ の適切な固有値の点を結ぶ必要がある。phononwebのサブルーチンを利用することで，ある $q$ と隣の $q$ のそれぞれの固有ベクトルを比較して，適切な固有値の点を繋げるように，固有値のデータの並び替えを行う。
