...
QEでの分散の計算で出力されるデータには，各波数 $q$ における固有値が書き込まれる。ところが，この固有値のデータは単に大きさ順に並んでいるだけである。適切な分散の図を得るためには，ある $q$ と隣の $q$ の適切な固有値の点を結ぶ必要がある。ckqetoolsでは，phononwebのサブルーチンを利用することである $q$ と隣の $q$ のそれぞれの固有ベクトルを比較し，適切な固有値の点を繋げるように固有値のデータの並び替えを行う。

`--disable-reorder` オプションを付けると固有値の並び替えをせず，単に固有値の大きさ順に線を繋げただけになる。
